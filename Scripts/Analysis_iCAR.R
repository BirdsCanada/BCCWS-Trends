...
for(i in 1:length(sp.list)){
  ...
  if(nrow(dat)>0){        
    ...
    if(min.data==TRUE){
      ...
      #Get the median observation count
      dat <- dat %>%
        group_by(SurveyAreaIdentifier, wyear) %>%
        mutate(ObservationCount = round(median(ObservationCount, na.rm = TRUE))) %>%
        ungroup()
     ...
     #Dispersion Statistic
       ...
       write.table(dispersion_entry,
                   file = paste0(out.dir, "BCCWS_DispersionStat.csv"),
                   append = TRUE,
                   sep = ",",
                   row.names = FALSE,
                   col.names = FALSE)
     ...
     tmp1 <- tmp0 %>%
      ...
      tmp2_area <- area_weighted_indices %>%
        ...
        mutate(
          ...
          Area = 999,
          Name = "AreaWeighted", 
          area_code="BC" # Identifier for merged output
         )
      ...
      write.table(write.trend, 
                  file = paste(out.dir, "BCCWS_TrendsSlope.csv", sep = ""), 
                  row.names = FALSE, 
                  append = TRUE, 
                  quote = FALSE, 
                  sep = ",", 
    ...
    ##Calculate trend_index for plotting on NatureCounts
      ...
      slope_points_summary <- area_slope_points %>%
        ...
        mutate(area_code = "BC", year=wyear) %>% select(-wyear)
      ...
      write.table(indices.csv,
                  file = paste(out.dir,	"BCCWS_AnnualIndices.csv", sep = ""),
                  row.names = FALSE,
                  append = TRUE,
                  quote = FALSE,
                  sep = ",",
