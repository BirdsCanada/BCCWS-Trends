...
in.BCCWS <- read.csv("Data/BCCWS.csv") # reads in back-up copy of database 
  ...
  in.BCCWS$wmonth <-in.BCCWS$wmonth <- ifelse(in.BCCWS$MonthCollected >= 9, 
                                              in.BCCWS$MonthCollected - 8, 
  ...
  in.BCCWS <- in.BCCWS %>% dplyr::select(ProjectCode, SurveyAreaIdentifier, SpeciesCode, species_id, CommonName, ObservationCount,  wyear, YearCollected, wmonth, MonthCollected, DayCollected)
  ...
  sample<-sample %>% filter(CommonName != "") %>% dplyr::select(-n_tot)
  ...
  # Now, create specie_id based on the new CommonName
  in.BCCWS <- in.BCCWS %>%
    mutate(
      species_id = case_match(
    CommonName,
    "Large Gull"              ~ 41291,
    "Greater-Lesser Scaup"    ~ 695,
    "Eared-Horned Grebe"      ~ 41201, 
    "Canada-Cackling Goose"   ~ 261,
    "Western-Clark's Grebe"   ~ 1641,
    # For any other species, their ID will be NA
    .default = species_id
  ))
